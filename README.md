...
```
Переходим с помощью команды `cd <name_reposytory>` в данный репозиторий и производим работы с данным репозиторием. По правилу хорошего тона необходимо вносить изменения в созданной **новой** ветке. Далее необходимо закоммитить все изменения и с помощью команды `git push -u origin <name_new_branch>` отправить на свою страницу.